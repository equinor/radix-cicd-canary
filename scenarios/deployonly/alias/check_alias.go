...
func DefaultResponding(ctx context.Context, cfg config.Config, suiteName string) error {
	...
	publicDomainName, err := test.WaitForCheckFuncWithValueOrTimeout(cfg, func(cfg config.Config, ctx context.Context) (string, error) {
		return application.TryGetPublicDomainName(cfg, appName, defaults.App3EnvironmentName, defaults.App3Component1Name)
	}, appCtx)
	if err != nil {
		return err
	}
	...
	canonicalDomainName, err := test.WaitForCheckFuncWithValueOrTimeout(cfg, func(cfg config.Config, ctx context.Context) (string, error) {
		return application.TryGetCanonicalDomainName(cfg, appName, defaults.App3EnvironmentName, defaults.App3Component1Name)
	}, appCtx)
	if err != nil {
		return err
	}
	...
	err = test.WaitForCheckFuncOrTimeout(cfg, func(_ config.Config, ctx context.Context) error {
		schema := "https"
